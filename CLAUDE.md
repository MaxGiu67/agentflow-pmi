<!-- Aggiornato: 2026-04-07 — Pivot 10: Portal Integration + Data-driven Products -->
# CLAUDE.md
...
| Pivot 10 (Sprint 42-45) | 12 | — | Portal Integration, Data-driven Products, Pipeline CRUD, Account Manager matching |
...
### CRM Sales + Email Marketing (Pivot 7-10)
...
- `api/modules/crm_products/` — Prodotti CRM per tenant (data-driven, no hardcoded), `pipeline_template_id` dropdown
...
- `api/modules/pipeline_templates/` — Pipeline templates **full CRUD** (create/update/delete template + stages indipendenti) + seed per tenant
...
### Integrazione PortalJS.be (Pivot 10 — ADR-011) — COMPLETATO
- `api/adapters/portal_client.py` — Client async per PortalJS.be API (NestJS + Prisma + PostgreSQL), JWT HS256 auto-generato
...
- **23+ proxy endpoint**: `/api/v1/portal/*` (customers, persons, projects, timesheets, match-customer, batch-match, link-project, deal-project, deal-progress, project-types, locations, account-managers, my-account-manager)
- Offer creation con full Portal DTO (project_code, name, customer_id, accountManager_id, project_type_id, location_id, billing_type)
- Account manager auto-match by email
...
### Pipeline Templates (Pivot 9-10)
- `PipelineTemplate` — template FSM (code, name, pipeline_type: services/product/custom) — **full CRUD**
- `PipelineTemplateStage` — stati per template (required_fields JSON, sla_days, is_optional) — **CRUD indipendente**
- `crm_products.pipeline_template_id` — prodotto determina pipeline (data-driven, no hardcoded)
...
| **PortalJS.be** | Gestione operativa: commesse, rapportini, dipendenti (NestJS + Prisma). JWT HS256, 23+ proxy endpoint. | **Attivo (Pivot 10)** |
...
- Stories Pivot 10: `specs/03-user-stories-pivot9.md` (sezione Pivot 10, US-230 -> US-241)
- Implementation: `specs/07-implementation.md`
- Sprint Plan Pivot 10: `specs/05-sprint-plan-pivot9.md` (Sprint 42-45)
- ADR-009: `specs/technical/ADR-009-crm-interno-brevo.md`
- ADR-011: `specs/technical/ADR-011-portal-integration.md`
- Portal Tech Spec: `specs/technical/portal-integration.md`
